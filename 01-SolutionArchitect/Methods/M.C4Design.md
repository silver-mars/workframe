...
The aim is to make design options explicit and discussable before detailed implementation decisions are locked in.
## Why it matters
...
- **Component** - selected internal responsibilities within a container when they carry material architectural risk or decision weight.
Use a component view selectively. Do not create it merely to complete the C4 hierarchy.
Code-level diagrams are outside the default scope of this method. Add them only when implementation structure is itself
## Inputs to clarify
...
**Failure domain** - a scope within which a single failure can propagate without crossing an explicit isolation boundary.
...
**Change driver** - a force (business, technical regulatory, operational) that is likely to require architectural change.
**Trade-off** - a design decision where improving one quality attribute or concern reduces another.
## Example cases
