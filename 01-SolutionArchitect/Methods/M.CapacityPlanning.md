...
In this context, **important** means that the current capacity is insufficient, so you need to estimate when the required capacity will become available, or take action to free up  For example, this could be to run fstrim for cleaning disk storage.
