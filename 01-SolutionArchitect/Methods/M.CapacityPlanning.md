...
In this context, **important** means that the current capacity is insufficient, so you need to estimate when the required capacity will become available, or take action to free up existing capacity. For example, this may involve running fstrim to reclaim disk space.
