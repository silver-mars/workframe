...
securityContext:
  ...
  seccompProfile:
    type: RuntimeDefault
```
At system level, these settings should be reinforced by admission controls and platform policies so that the baseline remains enforceable across workloads rather than depending on local developer 
